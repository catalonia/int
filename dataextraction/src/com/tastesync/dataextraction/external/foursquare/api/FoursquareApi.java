 ...
 */
package com.tastesync.dataextraction.external.foursquare.api;

import com.tastesync.dataextraction.external.foursquare.api.entities.Badge;
import com.tastesync.dataextraction.external.foursquare.api.entities.BadgeSets;
import com.tastesync.dataextraction.external.foursquare.api.entities.Badges;
import com.tastesync.dataextraction.external.foursquare.api.entities.Category;
import com.tastesync.dataextraction.external.foursquare.api.entities.Checkin;
import com.tastesync.dataextraction.external.foursquare.api.entities.CheckinGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.Comment;
import com.tastesync.dataextraction.external.foursquare.api.entities.CompactUser;
import com.tastesync.dataextraction.external.foursquare.api.entities.CompactVenue;
import com.tastesync.dataextraction.external.foursquare.api.entities.CompleteSpecial;
import com.tastesync.dataextraction.external.foursquare.api.entities.CompleteTip;
import com.tastesync.dataextraction.external.foursquare.api.entities.CompleteUser;
import com.tastesync.dataextraction.external.foursquare.api.entities.CompleteVenue;
import com.tastesync.dataextraction.external.foursquare.api.entities.KeywordGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.LeaderboardItemGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.LinkGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.Menu;
...
import com.tastesync.dataextraction.external.foursquare.api.entities.RecommendationGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.Recommended;
import com.tastesync.dataextraction.external.foursquare.api.entities.Setting;
import com.tastesync.dataextraction.external.foursquare.api.entities.SpecialGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.TipGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.Todo;
import com.tastesync.dataextraction.external.foursquare.api.entities.TodoGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.UserGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.VenueGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.VenueHistoryGroup;
import com.tastesync.dataextraction.external.foursquare.api.entities.VenuesSearchResult;
import com.tastesync.dataextraction.external.foursquare.api.entities.Warning;
import com.tastesync.dataextraction.external.foursquare.api.entities.notifications.Notification;
import com.tastesync.dataextraction.external.foursquare.api.io.DefaultIOHandler;
import com.tastesync.dataextraction.external.foursquare.api.io.IOHandler;
import com.tastesync.dataextraction.external.foursquare.api.io.Method;
import com.tastesync.dataextraction.external.foursquare.api.io.MultipartParameter;
import com.tastesync.dataextraction.external.foursquare.api.io.Response;
...
public class FoursquareApi {
    ...
    public Result<CompleteUser> user(String userId)
        ...
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (CompleteUser) JSONFieldParser.parseEntity(CompleteUser.class,
                        response.getResponse().getJSONObject("user"),
    ...
    public Result<LeaderboardItemGroup> usersLeaderboard(Integer neighbors)
        ...
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (LeaderboardItemGroup) JSONFieldParser.parseEntity(LeaderboardItemGroup.class,
                        response.getResponse().getJSONObject("leaderboard"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<Badges> usersBadges(String userId)
        ...
        try {
            ...
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "users/" + userId + "/badges", true);
            Badges result = null;

            if (response.getMeta().getCode() == 200) {
                BadgeSets sets = (BadgeSets) JSONFieldParser.parseEntity(BadgeSets.class,
                        response.getResponse().getJSONObject("sets"),
                        this.skipNonExistingFields);
                Badge[] badges = (Badge[]) JSONFieldParser.parseEntitiesHash(Badge.class,
                        response.getResponse().getJSONObject("badges"),
                        this.skipNonExistingFields);
                String defaultSetType = response.getResponse()
                                                .getString("defaultSetType");
    ...
    public Result<CheckinGroup> usersCheckins(String userId, Integer limit,
        ...
        try {
            ...
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "users/" + userId + "/checkins", true, "limit", limit,
                    "offset", offset, "afterTimestamp", afterTimestamp,
                    "beforeTimestamp", beforeTimestamp);
            CheckinGroup result = null;

            if (response.getMeta().getCode() == 200) {
                result = (CheckinGroup) JSONFieldParser.parseEntity(CheckinGroup.class,
                        response.getResponse().getJSONObject("checkins"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<TipGroup> usersTips(String userId, String sort, String ll,
        ...
        try {
            ...
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "users/" + userId + "/tips", true, "sort", sort, "ll", ll,
    ...
    public Result<TodoGroup> usersTodos(String userId, String sort, String ll)
        ...
        try {
            ...
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "users/" + userId + "/todos", true, "sort", sort, "ll", ll);
            TodoGroup result = null;

            if (response.getMeta().getCode() == 200) {
                result = (TodoGroup) JSONFieldParser.parseEntity(TodoGroup.class,
    ...
    public Result<VenueHistoryGroup> usersVenueHistory(String userId,
        ...
        try {
            ...
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "users/" + userId + "/venuehistory", true,
                    "beforeTimestamp", beforeTimestamp, "afterTimestamp",
                    afterTimestamp);
            VenueHistoryGroup result = null;

            if (response.getMeta().getCode() == 200) {
                result = (VenueHistoryGroup) JSONFieldParser.parseEntity(VenueHistoryGroup.class,
                        response.getResponse().getJSONObject("venues"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<CompleteUser> usersSetPings(String userId, String value)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
                    "users/" + userId + "/setpings", true, "value", value);
            CompleteUser result = null;
    ...
    public Result<CompactUser[]> usersSearch(String phone, String email,
        ...
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (CompactUser[]) JSONFieldParser.parseEntities(CompactUser.class,
    ...
    public Result<UserGroup> usersFriends(String userId)
        ...
        try {
            ...
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "users/" + userId + "/friends", true);
            UserGroup result = null;

            if (response.getMeta().getCode() == 200) {
                result = (UserGroup) JSONFieldParser.parseEntity(UserGroup.class,
     ...
     * @throws FoursquareApiException when something unexpected happens
     */
    public Result<CompleteVenue> venue(String venueId, String YYYYMMDD)
        throws FoursquareApiException {
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "venues/" + venueId, isAuthenticated(), "v", YYYYMMDD);
            CompleteVenue result = null;

            System.out.println("response=" + response.getResponse());

            System.out.println("venueId=" + venueId + " v=" + YYYYMMDD);

            if (response.getMeta().getCode() == 200) {
                result = (CompleteVenue) JSONFieldParser.parseEntity(CompleteVenue.class,
                        response.getResponse().getJSONObject("venue"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<Recommended> venuesExplore(String ll, Double llAcc,
        ...
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                KeywordGroup keywords = (KeywordGroup) JSONFieldParser.parseEntity(KeywordGroup.class,
                        response.getResponse().getJSONObject("keywords"),
                        this.skipNonExistingFields);
                RecommendationGroup[] groups = (RecommendationGroup[]) JSONFieldParser.parseEntities(RecommendationGroup.class,
                        response.getResponse().getJSONArray("groups"),
                        this.skipNonExistingFields);
                Warning warning = response.getResponse().has("warning")
                    ? (Warning) JSONFieldParser.parseEntity(Warning.class,
                        response.getResponse().getJSONObject("warning"),
    ...
    public Result<CheckinGroup> venuesHereNow(String venueId, Integer limit,
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "venues/" + venueId + "/herenow", isAuthenticated(),
                    "limit", limit, "offset", offset, "afterTimestamp",
                    afterTimestamp);
            CheckinGroup result = null;

            if (response.getMeta().getCode() == 200) {
                result = (CheckinGroup) JSONFieldParser.parseEntity(CheckinGroup.class,
                        response.getResponse().getJSONObject("hereNow"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<TipGroup> venuesTips(String venueId, String sort,
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "venues/" + venueId + "/tips", isAuthenticated(), "sort",
    ...
    public Result<Menu> venuesMenus(String venueId, String YYYYMMDD)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "venues/" + venueId + "/menu", isAuthenticated(), "v",
                    YYYYMMDD);
            Menu result = null;

            if (response.getMeta().getCode() == 200) {
                result = (Menu) JSONFieldParser.parseEntity(Menu.class,
                        response.getResponse().getJSONObject("menu"),
                        this.skipNonExistingFields);
            }

            return new Result<Menu>(response.getMeta(), result);
        } catch (JSONException e) {
            throw new FoursquareApiException(e);
        }
    }
    ...
    public Result<PhotoGroup> venuesPhotos(String venueId, String group)
        throws FoursquareApiException {
        return venuesPhotos(venueId, group, null, null);
    }
    ...
    public Result<PhotoGroup> venuesPhotos(String venueId, String group,
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "venues/" + venueId + "/photos", isAuthenticated(),
                    "group", group, "limit", limit, "offset", offset);
            PhotoGroup result = null;

            if (response.getMeta().getCode() == 200) {
                result = (PhotoGroup) JSONFieldParser.parseEntity(PhotoGroup.class,
    ...
    public Result<LinkGroup> venuesLinks(String id)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "venues/" + id + "/links", isAuthenticated());
            LinkGroup result = null;

            if (response.getMeta().getCode() == 200) {
                result = (LinkGroup) JSONFieldParser.parseEntity(LinkGroup.class,
    ...
    public Result<Todo> venuesMarkTodo(String venuesId, String text)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
                    "venues/" + venuesId + "/marktodo", true, "text", text);
            Todo result = null;
    ...
    public Result<Object> venuesFlag(String id, String problem)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
                    "venues/" + id + "/flag", true, "problem", problem);
    ...
    public Result<Object> venuesProposeEdit(String id, String name,
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
                    "venues/" + id + "/proposeedit", true, "name", name,
                    "address", address, "crossStreet", crossStreet, "city",
                    city, "state", state, "zip", zip, "phone", phone, "ll", ll,
    ...
    public Result<Category[]> venuesCategories() throws FoursquareApiException {
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (Category[]) JSONFieldParser.parseEntities(Category.class,
    ...
    public Result<VenuesSearchResult> venuesSearch(String ll, Double llAcc,
        Double alt, Double altAcc, String query, Integer limit, String intent,
        String categoryId, String url, String providerId, String linkedId,
        String version) throws FoursquareApiException {
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
            ...
            if (response.getMeta().getCode() == 200) {
                ...
                if (response.getResponse().has("groups")) {
                    groups = (VenueGroup[]) JSONFieldParser.parseEntities(VenueGroup.class,
                            response.getResponse().getJSONArray("groups"),
                            this.skipNonExistingFields);
                }

                if (response.getResponse().has("venues")) {
                    venues = (CompactVenue[]) JSONFieldParser.parseEntities(CompactVenue.class,
                            response.getResponse().getJSONArray("venues"),
                            this.skipNonExistingFields);
                }
    ...
    public Result<VenuesSearchResult> venuesSearch(String providerId,
        String linkedId, String YYYYMMDD) throws FoursquareApiException {
        return venuesSearch(null, null, null, null, null, null, null, null,
            null, providerId, linkedId, YYYYMMDD);
    }

    public Result<VenuesSearchResult> venuesSearch(String ll, String query,
        String intent, String providerId, String linkedId, String YYYYMMDD)
        throws FoursquareApiException {
        return venuesSearch(ll, null, null, null, query, null, intent, null,
            null, null, null, YYYYMMDD);
    }

    public Result<VenuesSearchResult> venuesSearchWithOauthToken(
        String oauth_token, String linkedId, String YYYYMMDD)
        throws FoursquareApiException {
        try {
            ...
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "venues/search", isAuthenticated(), "providerId",
                    "factual", "linkedId", linkedId, "oauth_token",
                    oauth_token, "v", YYYYMMDD);
            VenuesSearchResult result = null;
    ...
    public Result<CompactVenue[]> venuesTrending(String ll, Integer limit,
        ...
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (CompactVenue[]) JSONFieldParser.parseEntities(CompactVenue.class,
                        response.getResponse().getJSONArray("venues"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<Checkin> checkin(String checkinId, String signature)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "checkins/" + checkinId, true, "signature", signature);
            Checkin result = null;
    ...
    public Result<Checkin> checkinsAdd(String venueId, String venue,
        String shout, String broadcast, String ll, Double llAcc, Double alt,
        Double altAcc) throws FoursquareApiException {
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
            ...
            if (response.getMeta().getCode() == 200) {
                result = (Checkin) JSONFieldParser.parseEntity(Checkin.class,
                        response.getResponse().getJSONObject("checkin"),
                        this.skipNonExistingFields);
                notifications = NotificationsParser.parseNotifications(response.getNotifications(),
    ...
    public Result<Checkin[]> checkinsRecent(String ll, Integer limit,
        ...
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (Checkin[]) JSONFieldParser.parseEntities(Checkin.class,
    ...
    public Result<Comment> checkinsAddComment(String checkinId, String text)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
                    "checkins/" + checkinId + "/addcomment", true, "text", text);
            Comment result = null;
    ...
    public Result<Checkin> checkinsDeleteComment(String checkinId,
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
                    "checkins/" + checkinId + "/deletecomment", true,
                    "commentId", commentId);
            Checkin result = null;
    ...
    public Result<CompleteTip> tip(String id) throws FoursquareApiException {
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (CompleteTip) JSONFieldParser.parseEntity(CompleteTip.class,
                        response.getResponse().getJSONObject("tip"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<CompleteTip[]> tipsSearch(String ll, Integer limit,
        ...
        try {
            ...
            if (response.getMeta().getCode() == 200) {
                result = (CompleteTip[]) JSONFieldParser.parseEntities(CompleteTip.class,
    ...
    public Result<Photo> photosAdd(String checkinId, String tipId,
        String venueId, String broadcast, String ll, Double llAcc, Double alt,
        Double altAcc, byte[] data) throws FoursquareApiException {
        try {
            ApiRequestResponse response = doApiMultipartMimeRequest("photos/add",
                    true, "checkinId", checkinId, "tipId", tipId, "venueId",
                    venueId, "broadcast", broadcast, "ll", ll, "llAcc", llAcc,
                    "alt", alt, "altAcc", altAcc,
    ...
    public Result<Setting> settingSet(String settingId, Boolean value)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.POST,
                    "settings/" + settingId + "/set", true, "value",
                    value ? 1 : 0);
            Setting result = null;
    ...
    public Result<CompleteSpecial> special(String id, String venueId)
        ...
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
                    "specials/" + id, isAuthenticated(), "venueId", venueId);
            CompleteSpecial result = null;

            if (response.getMeta().getCode() == 200) {
                result = (CompleteSpecial) JSONFieldParser.parseEntity(CompleteSpecial.class,
                        response.getResponse().getJSONObject("special"),
                        this.skipNonExistingFields);
            }
    ...
    public Result<SpecialGroup> specialsSearch(String ll, Double llAcc,
        Double alt, Double altAcc, Integer limit) throws FoursquareApiException {
        try {
            ApiRequestResponse response = doApiRequest(Method.GET,
            ...
            if (response.getMeta().getCode() == 200) {
                result = (SpecialGroup) JSONFieldParser.parseEntity(SpecialGroup.class,
                        response.getResponse().getJSONObject("specials"),
                        this.skipNonExistingFields);
            }
    ...
    public String getAuthenticationUrl() {
        return new StringBuilder(
            "https://foursquare.com/oauth2/authenticate?client_id=").append(this.clientId)
    ...
    public void authenticateCode(String code) throws FoursquareApiException {
        StringBuilder urlBuilder = new StringBuilder(
                "https://foursquare.com/oauth2/access_token?client_id=").append(this.clientId)
    ...
    private ApiRequestResponse handleCallbackApiResponse(Response response)
        ...
        if (response.getResponseCode() == 200) {
            ...
            JSONObject responseObject = new JSONObject(responseContent.substring(
            ...
            JSONArray notificationsJson = responseObject.optJSONArray(
                    "notifications");
